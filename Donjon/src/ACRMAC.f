*DECK ACRMAC
      SUBROUTINE ACRMAC(IPMAC,IPAPX,IACCS,NMIL,NMIX,NGRP,IMPX,NCAL,
     1 NSURFD,MIXC,TERP,LPURE,B2)
*
*-----------------------------------------------------------------------
*
*Purpose:
...
*----
     ...
     & ITYPE, LENGTH, N1D, N2D, NDEL, NED, NF, NFTMP, NL, NLTMP, NTYPE
...
*----
  120 CALL LCMOP(IPTMP,'*ELEMENTARY*',0,1,0)
      CALL ACRCAL(IPAPX,IPTMP,ICAL,IMPX,NMIL,NGRP)
*----
...
*----
      ...
      IF(IACCS.EQ.0) THEN
        ...
        NL=ISTATE(3)
        NF=ISTATE(4)
        IF(NF.GT.MAXNFI) CALL XABORT('ACRMAC: MAXNFI OVERFLOW(2).')
        NED=ISTATE(5)
        ITRANC=ISTATE(6)
        NDEL=ISTATE(7)
        IDF=ISTATE(12)
        IF(NED.GT.MAXED) CALL XABORT('ACRMAC: MAXED OVERFLOW(2).')
        CALL LCMGTC(IPTMP,'ADDXSNAME-P0',8,NED,HVECT)
        N1D=8+NED+NL
        N2D=2*(NDEL+1)
        IF(NL.GT.MAXNL) CALL XABORT('ACRMAC: MAXNL OVERFLOW(2).')
  ...
  150   CONTINUE
      ...
      ELSE
        NLTMP=ISTATE(3)
        NFTMP=ISTATE(4)
        IF(NLTMP.GT.MAXNL) CALL XABORT('ACRMAC: MAXNL OVERFLOW(3).')
        IF(NFTMP.GT.MAXNFI) CALL XABORT('ACRMAC: MAXNFI OVERFLOW(3).')
        NL=MAX(NL,NLTMP)
