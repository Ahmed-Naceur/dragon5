...
expansion method (NEM).\cite{nestle}

\vskip 0.08cm

The nodal expansion method is based on an expansion of the {\sl transverse integrated flux} in terms of polynomials defined
...
\item[\moc{CMFD}] keyword used to impose the coarse mesh finite difference method. Only polynomials $p_0(u)$ to $p_2(u)$ are used to expand the flux. By default,
the nodal expansion method is used.
...
\dusa{FLUX} \moc{:=} \moc{NSSF:} \dusa{TRACK} \dusa{MACRO} \moc{::} \dstr{NSSF\_data}
\end{DataStructure}
...
\item[\dusa{maxout}] maximum number of external iterations. The fixed default
value is \dusa{maxout} $=1000$.

\item[\dusa{epsout}] convergence criterion for the external iterations. The
fixed default value is \dusa{epsout} $=1.0\times 10^{-6}$.
