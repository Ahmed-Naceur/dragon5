...
* ENERGY UNDER CUTOFF INITIALIZATION
      DO IR=1,NREG
      DCUTOFF(IR)=0.0
      ENDDO
*----
...
*----
     ...
     1 ESTOPW(0:NBMIX,2,NGRP),EMOMTR(0:NBMIX,NGRP))
      NANI=1
      DO 10 II=1,NGEFF
...
*----
     ...
     1  //'GTH.')
        ...
        CALL LCMLEN(JPSYS,'DRAGON-EMOMTR',IMOMTR,ITYLCM)
        IF(IMOMTR.GT.0) CALL LCMGET(JPSYS,'DRAGON-EMOMTR',EMOMTR(0,IG))
      ENDIF
*----
*  PRINT SOURCES.
*----
...
*------------
   ...
   30   CONTINUE
     ...
     1    SGAR(0,IG),NCODE,ZCODE,QEXT(1,IG),LFIXUP,FUNKNO(1,IG),IG)
        ELSE
...
* TERME ANGULAIRE DE FOKKER-PLANCK (CALIBRATION)      
        IF(IMOMTR.GT.0) THEN 
          ALP=2*EMOMTR(1,IG)
        ELSE
          ALP=0.0
        ...
        CALL SNFP1P(NREG,NBMIX,ISCHM,IELEM,IBFP,NLF,NSCT,U,W,PL,
     1  MAT,VOL,SGAR(0,IG),ESTOPW(0,1,IG),NCODE,ZCODE,DELTAE(IG),
     2  QEXT(1,IG),QEXT(IOF,IG),LFIXUP,FUNKNO(1,IG),FUNKNO(IOF,IG),
     3  ALP,IG)


* UNDER CUTOFF ENERGY DEPOSITION TERM
       IF(IG.EQ.NGRP) THEN
       ...
       ENDDO
       IBM=MAT(IR)
       DCUTOFF(IR)=ECUTOFF*ESTOPW(IBM,2,IG)
     1  *FLUXP
   71  CONTINUE      
       ENDIF
*-----------------------------------
...
*
  ...
  240   CONTINUE
     ...
     2    QEXT,LFIXUP,DU,DE,W,MRM,MRMY,DB,DA,PL,FUNKNO)
...
* UNDER CUTOFF ENERGY DEPOSITION TERM
      ...
      DO 241 IR=1,NREG
       FLUXP=0.0
       DO IP=1,NLF
       IND=(IR-1)*NLF*IELEM*IELEM+(IP-1)*IELEM*IELEM
     1 +IELEM*IELEM*NSCT*NREG+1
       JND=(IP-1)*NSCT+1
       FLUXP=FLUXP+W(IP)*FUNKNO(IND,IG)*PL(JND)
       ENDDO
       IBM=MAT(IR)
...
* UNDER CUTOFF ENERGY DEPOSITION TERM
       ...
       FLUXP=0.0
       DO IP=1,NLF
       IND=(IR-1)*NLF*IELEM*IELEM*IELEM+(IP-1)*IELEM*IELEM*IELEM
     1 +IELEM*IELEM*IELEM*NSCT*NREG+1
       JND=(IP-1)*NSCT+1
       FLUXP=FLUXP+W(IP)*FUNKNO(IND,IG)*PL(JND)
       ENDDO
       IBM=MAT(IR)
