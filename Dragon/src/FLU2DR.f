...
*----
      PARAMETER(NSTATE=40)
      TYPE(C_PTR) IPREB,J1,JPSOU,JPFLUX,JPMACR,KPMACR,JPSYS,KPSYS,
     1 IPSTR,JPSTR,KPSTR,JPFLUP1,JPFLUP2
...
*----
  ...
  150 CONTINUE
      ...
      IF((.NOT.LSCAL).AND.(ILEAK.EQ.0)) THEN
         IDIR=0
         CALL DOORFVR(CXDOOR,JPSYS,NPSYS,IPTRK,IFTRAK,IPRT,NGRP,
     1   NMAT,IDIR,NREG,NUNKNO,IPHASE,LEXAC,MATCOD,VOL,KEYFLX,TITLE,
     2   FLUX(1,1,8),FLUX(1,1,7),IPREB,REBFLG,DCUTOFF,IPSOU)
     ...
     2   ILEAK,B2,DIFF,GAMMA,FLUX(1,1,6),IPREB,REBFLG,DCUTOFF,IPSOU)
      ELSE
...
*
     ...
     2     TITLE,ILEAK,B2,DIFF,GAMMA,FLUX(1,1,6),IPREB,REBFLG,DCUTOFF,
     3     IPSOU)
         ENDIF
  230    CONTINUE
      ENDIF
...
*----
  ...
  510 CONTINUE
      ...
      IF(ILEN.GT.0) THEN
        CALL LCMGET(IPSOU,'NORM-FS',NORM)
        CALL LCMPUT(IPFLUX,'NORM-FS',1,2,NORM)
      ENDIF
      ENDIF
