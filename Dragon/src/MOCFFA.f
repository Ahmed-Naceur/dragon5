*DECK MOCFFA
      SUBROUTINE MOCFFA(SUBSCH,NR,NS,NUN,MT,LINE,SEGLEN,NRSEG,NE,NF,
...
* Ray-tracing (anisotropic scattering case,'source term isolation' on).
*
*Copyright:
...
*           phi_k
            TEMP       = FL1(IE)
*           phi_{k+1}
            FL1(IE)    = TEMP * EXPT(IE,IL) 
     1                  + FLP(IE,IL) * EXP2(IE,IL)
*           phi_k * ((1 - exp(-tau_k)) / tau_k)
            FLP(IE,IL) = TEMP * EXP2(IE,IL)
...
*           - direction
            TEMP       = FL0(IE)
            FL0(IE)    = TEMP * EXPT(IE,JL) 
     1                  + FLM(IE,JL) * EXP2(IE,JL)
            FLM(IE,JL) = TEMP * EXP2(IE,JL)
