*DECK PNF
      SUBROUTINE PNF(IPSYS,NPSYS,IPTRK,IFTRAK,IMPX,NGRP,IDIR,NREG,NBMIX,
...
*----
      IF(IDIR.NE.0) CALL XABORT('PNF: EXPECTING IDIR=0')
      IF(IFTRAK.NE.0) CALL XABORT('PNF: EXPECTING IFTRAK=0')
      IF(IMPX.GT.3) THEN
        WRITE(IUNOUT,'(//6H PNF: ,A72)') TITR
        ALLOCATE(FGAR(NREG))
        DO IG=1,NGRP
...
*----
      CALL PNFLV(KPSYS,INCONV,INGIND,IPTRK,IMPX,NGRP,NGEFF,NREG,NBMIX,
