...
*----
     ...
     1 ISBS,NBS,ISBSM(2*ISBS,NLF*ISBS)
     ...
     3 FLUX0(IELEM,NLF,NREG),BS(NBS*ISBS)
      LOGICAL LFIXUP
*----
*  LOCAL VARIABLES
*----
...
*----
         IF(ISBS.EQ.1.AND.ISBSM(2,IP).NE.0) THEN
           AFB=AFB+BS(ISBSM(2,IP))
         ENDIF
         DO 30 I=NREG,1,-1
            IBM=MAT(I)
            DO 25 IEL=1,IELEM
...
*             Przybylski and Ligou type
            ...
            IF(IELEM.EQ.1) THEN
              Q2(1,2)=(FN*FEP(1)+Q(1))*VOL(I)-2.0D0*U(IP)*AFB
     ...
     1        U(IP)*AFB
            ELSE IF(IELEM.EQ.3) THEN
              ...
              Q2(1,4)=(FN*FEP(1)+Q(1))*VOL(I)-2.0D0*U(IP)*AFB
              Q2(2,1)=Q2(1,2)
              Q2(2,2)=-(FD+TOTAL(IBM))*VOL(I)
              Q2(2,3)=-2.0D0*SQRT(15.0D0)*U(IP)
              Q2(2,4)=-(FN*FEP(2)+Q(2))*VOL(I)
              Q2(3,1)=Q2(1,3)
              Q2(3,2)=Q2(2,3)
              Q2(3,3)=(FD+TOTAL(IBM))*VOL(I)-10.0D0*U(IP)
              Q2(3,4)=(FN*FEP(3)+Q(3))*VOL(I)-2.0D0*SQRT(5.0D0)*
     1        U(IP)*AFB
            ENDIF
...
*----
         ...
         ENDIF
         IF(ISBS.EQ.1.AND.ISBSM(1,NLF-IP+1).NE.0) THEN
           AFB=AFB+BS(ISBSM(1,NLF-IP+1))
         ENDIF
         DO 50 I=1,NREG
            IBM=MAT(I)
            DO 45 IEL=1,IELEM
...
*             Przybylski and Ligou type
            ...
            FEP(:IELEM)=QEXT0(:IELEM,NLF/2+IP,I)
            IF(IELEM.EQ.1) THEN
              Q2(1,2)=(FN*FEP(1)+Q(1))*VOL(I)+2.0D0*U(NLF-IP+1)*AFB
     ...
     1        U(NLF-IP+1)*AFB
            ELSE IF(IELEM.EQ.3) THEN
              ...
              Q2(1,4)=(FN*FEP(1)+Q(1))*VOL(I)+2.0D0*U(NLF-IP+1)*AFB
              Q2(2,1)=Q2(1,2)
              Q2(2,2)=-(FD+TOTAL(IBM))*VOL(I)
...
*             Przybylski and Ligou type
     ...
     >         3**(0.5D0)*5**(0.5D0)*U(IP) 
            ...
            CALL ALSBD(IELEM,1,Q2,IER,IELEM)
            IF(IER.NE.0) CALL XABORT('SNFT1P: SINGULAR MATRIX(7).')
...
*             Przybylski and Ligou type
     ...
     >         5**(0.5D0)*7**(0.5D0)*U(NLF-IP+1) 
            ...
            CALL ALSBD(IELEM,1,Q2,IER,IELEM)
            IF(IER.NE.0) CALL XABORT('SNFT1P: SINGULAR MATRIX(8).')
...
*             Przybylski and Ligou type
     ...
     >         3**(0.5D0)*5**(0.5D0)*U(IP) 
            ...
            CALL ALSBD(IELEM,1,Q2,IER,IELEM)
            IF(IER.NE.0) CALL XABORT('SNFT1P: SINGULAR MATRIX(9).')
...
*             Przybylski and Ligou type
     ...
     >         5**(0.5D0)*7**(0.5D0)*U(NLF-IP+1) 
            ...
            CALL ALSBD(IELEM,1,Q2,IER,IELEM)
            IF(IER.NE.0) CALL XABORT('SNFT1P: SINGULAR MATRIX(10).')
...
*         Przybylski and Ligou type
     ...
     >      3**(0.5D0)*5**(0.5D0)*U(IP) 
         ...
         CALL ALSBD(IELEM,1,Q2,IER,IELEM)
         IF(IER.NE.0) CALL XABORT('SNFT1P: SINGULAR MATRIX(11).')
...
*          Przybylski and Ligou type
     ...
     >      5**(0.5D0)*7**(0.5D0)*U(NLF-IP+1) 
         ...
         CALL ALSBD(IELEM,1,Q2,IER,IELEM)
         IF(IER.NE.0) CALL XABORT('SNFT1P: SINGULAR MATRIX(12). ' )
