*DECK SNFLUX
      SUBROUTINE SNFLUX (KPSYS,INCONV,INGIND,IPTRK,IPSOUR,IMPX,NGRP,
     1 NGEFF,NREG,NBMIX,NUN,MAT,VOL,KEYFLX,FUNKNO,SUNKNO,ITER,DCUTOFF)
*
*-----------------------------------------------------------------------
*
*Purpose:
...
*----
      ...
      TYPE(C_PTR) JPSYS,JPSOUR,KPSOUR,KPSOUR_INFO
      INTEGER     IPAR(NSTATE),NCODE(6)
      REAL        ZCODE(6),SIDE,ECUTOFF
...
*----
      ALLOCATE(QEXT(NUN,NGRP),OLD(NUN,NGRP),IDL(NREG),DELTAE(NGRP))
*----
...
*----
      IF(IBFP.GT.0) THEN
        CALL LCMLEN(JPSYS,'DRAGON-ESTOP',IFP,ITYLCM)
        IF(IFP.NE.(NBMIX+1)*2) CALL XABORT('SNFLUX: INVALID ESTOPW LEN'
     1  //'GTH.')
        CALL LCMGET(JPSYS,'DRAGON-ESTOP',ESTOPW(0,1,IG))
        CALL LCMGET(JPSYS,'DRAGON-DELTE',DELTAE(IG))
        CALL LCMGET(JPSYS,'DRAGON-ECUTOFF',ECUTOFF)
      ENDIF
*----
*  INITIALIZATION FOR ENERGY DEPOSITION UNDER CUTOFF
*----
      DCUTOFF=0.0
*----
*  RECOVER INFORMATION ABOUT BOUNDARY SOURCES
*----
      IF(C_ASSOCIATED(IPSOUR)) THEN
     ...
     1 //' GEOMETRY.')
      ...
      CALL LCMGET(IPSOUR,'NBS',NBS)
      ALLOCATE(BSINFO(3,NBS),BS(MAXL,NBS))
      JPSOUR=LCMGID(IPSOUR,'DBSOUR')
      KPSOUR_INFO=LCMGIL(JPSOUR,1)
      KPSOUR=LCMGIL(JPSOUR,2)
      DO N=1,NBS
        CALL LCMGDL(KPSOUR_INFO,N,BSINFO(1,N))
      ...
      ALLOCATE(ISBSM(NLF,4,NGRP))
        ...
        DO N=1,NBS
          ...
          ELSEIF(BSINFO(3,N).EQ.-2) THEN
            ISBSM(BSINFO(2,N),3,BSINFO(1,N))=N
          ELSEIF(BSINFO(3,N).EQ.2) THEN
            ISBSM(BSINFO(2,N),4,BSINFO(1,N))=N
          ENDIF
        ENDDO
      ELSE IF(ITYPE.EQ.7) THEN
      ALLOCATE(ISBSM(NLF,6,NGRP))
        ISBSM=0
        DO N=1,NBS
          IF(BSINFO(3,N).EQ.-1) THEN
            ISBSM(BSINFO(2,N),1,BSINFO(1,N))=N
          ...
          ELSEIF(BSINFO(3,N).EQ.2) THEN
            ISBSM(BSINFO(2,N),4,BSINFO(1,N))=N
          ELSEIF(BSINFO(3,N).EQ.-3) THEN
            ISBSM(BSINFO(2,N),5,BSINFO(1,N))=N
          ELSEIF(BSINFO(3,N).EQ.3) THEN
            ISBSM(BSINFO(2,N),6,BSINFO(1,N))=N
          ENDIF
        ENDDO
      ENDIF
      ...
      ELSE
        ...
        NBS=1
        MAXL=1
        ALLOCATE(BSINFO(3,1),BS(1,1),ISBSM(1,1,NGRP))
      ENDIF
...
*------------
     ...
     1  //'(1)')
        ...
        IG=INGIND(II)
        IF(IMPX.GT.1) WRITE(IUNOUT,510) IG,'SN/1D-slab'
   ...
   30   CONTINUE
     ...
     1    SGAR(0,IG),NCODE,ZCODE,QEXT(1,IG),LFIXUP,FUNKNO(1,IG),ISBS,
     2    NBS,ISBSM(1,1,IG),BS)
        ELSE
...
*------------
        ...
        IG=INGIND(II)
        IF(IMPX.GT.1) WRITE(IUNOUT,510) IG,'SN-BFP/1D-slab'
        QEXT(:NUN,IG)=SUNKNO(:NUN,IG)
        DO 70 IR=1,NREG
        IBM=MAT(IR)
   ...
   70   CONTINUE
     ...
     1  VOL,SGAR(0,IG),ESTOPW(0,1,IG),NCODE,ZCODE,DELTAE(IG),QEXT(1,IG),
     2  QEXT(IOF,IG),LFIXUP,FUNKNO(1,IG),FUNKNO(IOF,IG),ISBS,NBS,
     3  ISBSM(1,1,IG),BS)

*       UNDER CUTOFF ENERGY DEPOSITION TERM CALCULATION
        IF(IG.EQ.NGRP) THEN
        DO IR=1,NREG
        FLUXP=0.0
        DO IP=1,NLF
          IND=(IR-1)*NLF*IELEM+(IP-1)*IELEM+IELEM*NSCT*NREG+1
          JND=(IP-1)*NSCT+1
          FLUXP=FLUXP+W(IP)*FUNKNO(IND,IG)*PL(JND)
        ENDDO
        IBM=MAT(IR)
        DCUTOFF(IR)=ECUTOFF*ESTOPW(IBM,2,IG)*FLUXP
        ENDDO
        ENDIF
...
*
        ...
        IG=INGIND(II)
        IF(IMPX.GT.1) WRITE(IUNOUT,510) IG,'SN/1D-cyl'
        QEXT(:NUN,IG)=SUNKNO(:NUN,IG)
        DO 110 IR=1,NREG
        IBM=MAT(IR)
...
*
        ...
        IG=INGIND(II)
        IF(IMPX.GT.1) WRITE(IUNOUT,510) IG,'SN/1D-sph'
        QEXT(:NUN,IG)=SUNKNO(:NUN,IG)
        DO 140 IR=1,NREG
        IBM=MAT(IR)
...
*
        ...
        IG=INGIND(II)
        IF(IMPX.GT.1) WRITE(IUNOUT,510) IG,'SN/2D-car'
        QEXT(:NUN,IG)=SUNKNO(:NUN,IG)
        DO 190 IR=1,NREG
        IBM=MAT(IR)
  ...
  200   CONTINUE
        IF((ISCHM.EQ.1).AND.(IOMP.EQ.0))THEN
          CALL SNFT12(NUN,NGRP,NGEFF,IMPX,INCONV,INGIND,LX,LY,IELEM,
     1    NBMIX,NPQ,NSCT,MAT,VOL,SGAR,NCODE,ZCODE,QEXT,LFIXUP,DU,DE,W,
     2    MRM,MRMY,DB,DA,PL,FUNKNO,ISBS,NBS,ISBSM,BS,MAXL)
        ELSE IF((ISCHM.EQ.1).AND.(IOMP.GE.1))THEN
          CALL SNFD12(IOMP,NUN,NGRP,NGEFF,IMPX,INCONV,INGIND,LX,LY,
     1    IELEM,NBMIX,NPQ,NSCT,MAT,VOL,SGAR,NCODE,ZCODE,QEXT,LFIXUP,
     2    DU,DE,W,MRM,MRMY,DB,DA,PL,FUNKNO)
        ELSEIF((ISCHM.EQ.2).AND.(IOMP.EQ.0))THEN
          CALL SNFT22(NUN,NGRP,NGEFF,IMPX,INCONV,INGIND,LX,LY,IELEM,
     1    NBMIX,NPQ,NSCT,MAT,VOL,SGAR,NCODE,ZCODE,QEXT,DU,DE,W,MRM,
     2    MRMY,DB,DA,PL,FUNKNO,ISBS,NBS,ISBSM,BSM,MAXL)
        ELSE IF((ISCHM.EQ.2).AND.(IOMP.GE.1))THEN
          CALL SNFD22(IOMP,NUN,NGRP,NGEFF,IMPX,INCONV,INGIND,LX,LY,
     1    IELEM,NBMIX,NPQ,NSCT,MAT,VOL,SGAR,NCODE,ZCODE,QEXT,DU,DE,W,
     2    MRM,MRMY,DB,DA,PL,FUNKNO)
        ENDIF
      ELSE IF(ITYPE.EQ.5) THEN
*------------
...
*
        ...
        IG=INGIND(II)
        IF(IMPX.GT.1) WRITE(IUNOUT,510) IG,'SN-BFP/2D-car'
        QEXT(:NUN,IG)=SUNKNO(:NUN,IG)
        DO 240 IR=1,NREG
        IBM=MAT(IR)
  ...
  250   CONTINUE
        IF((ISCHM.EQ.1).AND.(IOMP.EQ.0)) THEN
          CALL SNFP12(NUN,NGRP,NGEFF,IMPX,INCONV,INGIND,LX,LY,IELEM,
     1    IBFP,NBMIX,NPQ,NSCT,MAT,VOL,SGAR,ESTOPW,NCODE,ZCODE,DELTAE,
     2    QEXT,LFIXUP,DU,DE,W,MRM,MRMY,DB,DA,PL,FUNKNO,DCUTOFF,ISBS,NBS,
     3    ISBSM,BS,MAXL)
        ELSE IF((ISCHM.EQ.1).AND.(IOMP.EQ.0)) THEN
          CALL SNFP22(NUN,NGRP,NGEFF,IMPX,INCONV,INGIND,LX,LY,IELEM,
     1    IBFP,NBMIX,NPQ,NSCT,MAT,VOL,SGAR,ESTOPW,NCODE,ZCODE,DELTAE,
     2    QEXT,DU,DE,W,MRM,MRMY,DB,DA,PL,FUNKNO,DCUTOFF,ISBS,NBS,
     3    ISBSM,BS,MAXL)
        ELSE
...
*       UNDER CUTOFF ENERGY DEPOSITION TERM CALCULATION
        DO IR=1,NREG
          IBM=MAT(IR)
          DCUTOFF(IR)=ECUTOFF*ESTOPW(IBM,2,NGRP)*DCUTOFF(IR)
        ENDDO
...
*
        ...
        IG=INGIND(II)
        IF(IMPX.GT.1) WRITE(IUNOUT,510) IG,'SN/2D-rz'
        QEXT(:NUN,IG)=SUNKNO(:NUN,IG)
        DO 280 IR=1,NREG
        IBM=MAT(IR)
...
*
        ...
        IG=INGIND(II)
        IF(IMPX.GT.1) WRITE(IUNOUT,510) IG,'SN/3D-car'
        QEXT(:NUN,IG)=SUNKNO(:NUN,IG)
        DO 330 IR=1,NREG
        IBM=MAT(IR)
  ...
  340   CONTINUE
        IF((ISCHM.EQ.1).AND.(IOMP.EQ.0)) THEN
          CALL SNFT13(NUN,NGRP,NGEFF,IMPX,INCONV,INGIND,LX,LY,LZ,IELEM,
     1    NBMIX,NPQ,NSCT,MAT,VOL,SGAR,NCODE,ZCODE,QEXT,LFIXUP,DU,DE,DZ,
     2    W,MRMX,MRMY,MRMZ,DC,DB,DA,PL,FUNKNO)
        ELSE IF((ISCHM.EQ.1).AND.(IOMP.GE.1)) THEN
          CALL SNFD13(IOMP,NUN,NGRP,NGEFF,IMPX,INCONV,INGIND,LX,LY,LZ,
     1    IELEM,NBMIX,NPQ,NSCT,MAT,VOL,SGAR,NCODE,ZCODE,QEXT,LFIXUP,DU,
     2    DE,DZ,W,MRMX,MRMY,MRMZ,DC,DB,DA,PL,FUNKNO)
        ELSE IF((ISCHM.EQ.2).AND.(IOMP.EQ.0)) THEN
          CALL SNFT23(NUN,NGRP,NGEFF,IMPX,INCONV,INGIND,LX,LY,LZ,IELEM,
     1    NBMIX,NPQ,NSCT,MAT,VOL,SGAR,NCODE,ZCODE,QEXT,DU,DE,DZ,W,MRMX,
     2    MRMY,MRMZ,DC,DB,DA,PL,FUNKNO)
        ELSE IF((ISCHM.EQ.2).AND.(IOMP.GE.1)) THEN
          CALL SNFD23(IOMP,NUN,NGRP,NGEFF,IMPX,INCONV,INGIND,LX,LY,LZ,
     1    IELEM,NBMIX,NPQ,NSCT,MAT,VOL,SGAR,NCODE,ZCODE,QEXT,DU,DE,DZ,
     2    W,MRMX,MRMY,MRMZ,DC,DB,DA,PL,FUNKNO)
        ENDIF
      ELSE IF(ITYPE.EQ.7) THEN
*------------
...
*
        ...
        IG=INGIND(II)
        IF(IMPX.GT.1) WRITE(IUNOUT,510) IG,'SN-BFP/3D-car'
        QEXT(:NUN,IG)=SUNKNO(:NUN,IG)
        DO 380 IR=1,NREG
        IBM=MAT(IR)
  ...
  390   CONTINUE
        IF((ISCHM.EQ.1).AND.(IOMP.EQ.0))THEN
          CALL SNFP13(NUN,NGRP,NGEFF,IMPX,INCONV,INGIND,LX,LY,LZ,IELEM,
     1    IBFP,NBMIX,NPQ,NSCT,MAT,VOL,SGAR,ESTOPW,NCODE,ZCODE,DELTAE,
     2    QEXT,LFIXUP,DU,DE,DZ,W,MRMX,MRMY,MRMZ,DC,DB,DA,PL,FUNKNO,
     3    DCUTOFF)
...
*
        ...
        IG=INGIND(II)
        IF(IMPX.GT.1) WRITE(IUNOUT,510) IG,'SN/2D-hex'
        QEXT(:NUN,IG)=SUNKNO(:NUN,IG)
        DO 430 IR=1,NREG
        IBM=MAT(IR)
  ...
  440   CONTINUE
        IF(ISCHM.EQ.1)THEN
          CALL SNFTH1(NUN,NGRP,NGEFF,IMPX,INCONV,INGIND,NHEX,IELEM,
     1    ISPLH,NBMIX,NPQ,NSCT,MAT,VOL,SGAR,QEXT,DU,DE,W,DB,DA,PL,
     2    CONNEC,IZGLOB,SIDE,CONFROM,FUNKNO)
        ELSEIF(ISCHM.EQ.2)THEN
          CALL SNFTH2(NUN,NGRP,NGEFF,IMPX,INCONV,INGIND,NHEX,IELEM,
     1    ISPLH,NBMIX,NPQ,NSCT,MAT,VOL,SGAR,QEXT,DU,DE,W,DB,DA,PL,
     2    CONNEC,IZGLOB,SIDE,CONFROM,FUNKNO)
        ENDIF
...
*
        ...
        IG=INGIND(II)
        IF(IMPX.GT.1) WRITE(IUNOUT,510) IG,'SN/3D-hex'
        QEXT(:NUN,IG)=SUNKNO(:NUN,IG)
        DO 480 IR=1,NREG
        IBM=MAT(IR)
  ...
  490   CONTINUE
        IF(ISCHM.EQ.1)THEN
          CALL SNFTH3(NUN,NGRP,NGEFF,IMPX,INCONV,INGIND,NHEX,LZ,IELEM,
     1    ISPLH,SIDE,NBMIX,NPQ,NSCT,MAT,VOL,NCODE,ZCODE,SGAR,QEXT,DU,
     2    DE,DZ,W,MRMZ,DC,DB,DA,PL,CONNEC,IZGLOB,CONFROM,FUNKNO)
        ELSEIF(ISCHM.EQ.2)THEN
          CALL SNFTH4(NUN,NGRP,NGEFF,IMPX,INCONV,INGIND,NHEX,LZ,IELEM,
     1    ISPLH,SIDE,NBMIX,NPQ,NSCT,MAT,VOL,NCODE,ZCODE,SGAR,QEXT,DU,
     2    DE,DZ,W,MRMZ,DC,DB,DA,PL,CONNEC,IZGLOB,CONFROM,FUNKNO)
        ENDIF
        DEALLOCATE(CONNEC,IZGLOB)
...
*----
      DO 495 II=1,NGEFF
         ...
         IF(IMPX.GT.5) THEN
            WRITE(IUNOUT,520) IG
            WRITE(IUNOUT,'(1P,4(5X,E15.7))') (FUNKNO(:,IG))
         ENDIF
  495 CONTINUE
*----
...
*----
     ...
     1  NBMIX,NUN,ISCAT,MAT,VOL,KEYFLX,IDL,NUNSA,IELEMSA,ZCODE,OLD,
     2  FUNKNO,NHEX)
      ENDIF
*----
