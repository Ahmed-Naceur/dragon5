...
*----
      ...
      INTEGER     IPAR(NSTATE),NCODE(6),BSINFO(2)
      REAL        ZCODE(6),SIDE
...
*------------
   ...
   30   CONTINUE
     ...
     1    SGAR(0,II),NCODE,ZCODE,QEXT(1,II),LFIXUP,FUNKNO(1,II),ISBS,
     2    SUM(NBS),ISBSM(1,1,II),BS)
        ELSE
          IF(ISBS.EQ.1) CALL XABORT('BOUNDARY SOURCES NOT IMPLEMENTED'
     1     //'WHEN DISABLING SHOOTING METHOD (USING NSHT KEYWORD).')
...
*------------
        IF(.NOT.PRESENT(FLUXC)) THEN
          CALL XABORT('SNFLUX: MISSING OPTIONAL ARGUMENT FLUXC(1).')
        ENDIF
        NSCT=ISCAT
   ...
   70   CONTINUE
     ...
     2  QEXT(IOF,II),LFIXUP,FUNKNO(1,II),FUNKNO(IOF,II),ISBS,SUM(NBS),
     3  ISBSM(1,1,II),BS) 
   ...
   72   CONTINUE
        FLUXC(IR)=ESTOPW(MAT(IR),2,II)*FLUXP
   71   CONTINUE     
        ENDIF
   80   CONTINUE
...
*
  ...
  200   CONTINUE
     ...
     2    DA,PL,FUNKNO,ISBS,SUM(NBS),ISBSM,BS,MAXL)
...
*------------
        IF(.NOT.PRESENT(FLUXC)) THEN
          CALL XABORT('SNFLUX: MISSING OPTIONAL ARGUMENT FLUXC(2).')
        ENDIF
        NSCT=ISCAT*(ISCAT+1)/2
        CALL LCMLEN(IPTRK,'DU',NPQ,ITYLCM)
...
*
  ...
  250   CONTINUE
     ...
     2    LFIXUP,DU,DE,W,MRM,MRMY,DB,DA,PL,FUNKNO,ISLG,FLUXC,ISBS,
     3    SUM(NBS),ISBSM,BS,MAXL)
        ELSE IF((ISCHM.EQ.2).AND.(IOMP.EQ.0)) THEN
          CALL SNFP22(NUN,NGEFF,IMPX,INCONV,NGIND,LX,LY,IELEM,
     1    IBFP,NBMIX,NPQ,NSCT,MAT,VOL,SGAR,ESTOPW,NCODE,ZCODE,DELTAE,
     2    QEXT,DU,DE,W,MRM,MRMY,DB,DA,PL,FUNKNO,ISLG,FLUXC,ISBS,
     3    SUM(NBS),ISBSM,BS,MAXL)
        ELSE
        ...
        DO 255 IR=1,NREG
        FLUXC(IR)=ESTOPW(MAT(IR),2,NGEFF)*FLUXC(IR)
  255   CONTINUE
        ENDIF
...
*
  ...
  340   CONTINUE
     ...
     2    MRMY,MRMZ,DC,DB,DA,PL,FUNKNO,ISBS,SUM(NBS),ISBSM,BS,MAXL)
        ELSE IF((ISCHM.EQ.1).AND.(IOMP.GE.1)) THEN
          IF(ISBS.EQ.1) CALL XABORT('BOUNDARY SOURCES NOT YET'
     1     //'IMPLEMENTED WITH DOMINO ALGORITHM.')
     ...
     2    MRMZ,DC,DB,DA,PL,FUNKNO,ISBS,SUM(NBS),ISBSM,BS,MAXL)
        ELSE IF((ISCHM.EQ.2).AND.(IOMP.GE.1)) THEN
          IF(ISBS.EQ.1) CALL XABORT('BOUNDARY SOURCES NOT YET'
     1     //'IMPLEMENTED WITH DOMINO ALGORITHM.')
...
*------------
        IF(.NOT.PRESENT(FLUXC)) THEN
          CALL XABORT('SNFLUX: MISSING OPTIONAL ARGUMENT FLUXC(3).')
        ENDIF
        NSCT=(ISCAT)**2
        CALL LCMLEN(IPTRK,'DU',NPQ,ITYLCM)
...
*
  ...
  390   CONTINUE
     ...
     1    NBMIX,NPQ,NSCT,MAT,VOL,SGAR,ESTOPW,NCODE,ZCODE,DELTAE,QEXT,
     2    LFIXUP,DU,DE,DZ,W,MRMX,MRMY,MRMZ,DC,DB,DA,PL,FUNKNO,ISLG,
     3    FLUXC,ISBS,SUM(NBS),ISBSM,BS,MAXL)
        ELSE IF((ISCHM.EQ.2).AND.(IOMP.EQ.0)) THEN
          CALL SNFP23(NUN,NGEFF,IMPX,INCONV,NGIND,LX,LY,LZ,IELEM,IBFP,
     1    NBMIX,NPQ,NSCT,MAT,VOL,SGAR,ESTOPW,NCODE,ZCODE,DELTAE,QEXT,
     2    DU,DE,DZ,W,MRMX,MRMY,MRMZ,DC,DB,DA,PL,FUNKNO,ISLG,FLUXC,ISBS,
     3    SUM(NBS),ISBSM,BS,MAXL)
        ELSE
          CALL XABORT('SNFLUX: TECHNIQUE NOT IMPLEMENTED.')
        ENDIF
...
*----
      DEALLOCATE(DELTAE,IDL,OLD,QEXT,BS,ISBSM)
      RETURN
*
