...
*----
      ...
      INTEGER ISTATE(NSTATE),XXX_LEN,YYY_LEN,ZZZ_LEN
      REAL ISOUR
...
! SOURCE INTENSITY KEYWORD
      ELSE IF(TEXT12.EQ.'IN') THEN
        ...
        ISOUR=FLOTT
        IF(ISOUR.LT.0.0) CALL XABORT('SOUR: INVALID INTENSITY.')
...
! OTHER VERIFICATIONS
      ...
      IF(ISXDEF.EQ.1.AND.(XMIN.LT.XXX(1).OR.XMAX.GT.XXX(XXX_LEN))) THEN
        CALL XABORT('SOUR: SOURCE X-LIMITS OUTSIDE THE GEOMETRY.')
      ENDIF
      IF(NDIM.GE.2) THEN
      IF(ISYDEF.EQ.1.AND.(YMIN.LT.YYY(1).OR.YMAX.GT.YYY(YYY_LEN))) THEN
        CALL XABORT('SOUR: SOURCE Y-LIMITS OUTSIDE THE GEOMETRY.')
      ENDIF
      IF(NDIM.GE.3) THEN
      IF(ISZDEF.EQ.1.AND.(ZMIN.LT.ZZZ(1).OR.ZMAX.GT.ZZZ(ZZZ_LEN))) THEN
     ...
     2 //' SOURCES.')
      ...
      ! Multiple boudary sources defined
      ! SOLUTION: Define vectors of [X- X+, Y-, Y+,...] et [XMIN1 XMIN2
      ! XMIN 3 ...] et [XMAX1 XMAX2 XMAX3] ...
