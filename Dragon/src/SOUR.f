...
*----
      ...
      TYPE(C_PTR) IPSOUROUT,IPMAC,IPTRK,IPGEOM !,IPSOURIN
      CHARACTER HSIGN*12,TEXT12*12
      INTEGER ISTATE(NSTATE),XXX_LEN,YYY_LEN,ZZZ_LEN,GN
...
*----
     ...
     1  '. L_GEOM EXPECTED.')
      ...
      IF(NDIM.EQ.1) THEN
      CALL LCMLEN(IPGEOM,'MESHX',XXX_LEN,ITYLCM)
      ALLOCATE(XXX(XXX_LEN))
      CALL LCMGET(IPGEOM,'MESHX',XXX)
      ELSE IF(NDIM.EQ.2) THEN
      CALL LCMLEN(IPGEOM,'MESHX',XXX_LEN,ITYLCM)
      CALL LCMLEN(IPGEOM,'MESHY',YYY_LEN,ITYLCM)
      ALLOCATE(XXX(XXX_LEN),YYY(YYY_LEN))
      CALL LCMGET(IPGEOM,'MESHX',XXX)
      CALL LCMGET(IPGEOM,'MESHY',YYY)
      ELSE IF(NDIM.EQ.3) THEN
      CALL LCMLEN(IPGEOM,'MESHX',XXX_LEN,ITYLCM)
      CALL LCMLEN(IPGEOM,'MESHY',YYY_LEN,ITYLCM)
      CALL LCMLEN(IPGEOM,'MESHZ',ZZZ_LEN,ITYLCM)
      ALLOCATE(XXX(XXX_LEN),YYY(YYY_LEN),ZZZ(ZZZ_LEN))
      CALL LCMGET(IPGEOM,'MESHX',XXX)
      CALL LCMGET(IPGEOM,'MESHY',YYY)
...
*----
*  RECOVER INPUT SOURCE INFORMATION
*----
...
! OTHER VERIFICATIONS
     ...
     1 //' X- OR X+ : XLIM SHOULD NOT BE DEFINED.')
      DO I=1,NSOUR
        IF(XMIN(I).LT.XXX(1).OR.XMAX(I).GT.XXX(XXX_LEN))
     ...
     1 //' Y- OR Y+ : YLIM SHOULD NOT BE DEFINED.')
      DO I=1,NSOUR
        IF(YMIN(I).LT.YYY(1).OR.YMAX(I).GT.YYY(YYY_LEN))
     ...
     1 //' Z- OR Z+ : ZLIM SHOULD NOT BE DEFINED.')
      DO I=1,NSOUR
        IF(ZMIN(I).LT.ZZZ(1).OR.ZMAX(I).GT.ZZZ(ZZZ_LEN))
...
*----
      ...
      IF(STYPE.EQ.0) THEN
         ALLOCATE(SUNKNO(NUNS,NG))
         SUNKNO(:NUNS,:NG)=0.0
         CALL SOURISO(IPTRK,IPGEOM,NREG,NG,NANIS,NUNS,NDIM,NSOUR,ISOUR,
     1   XMIN,XMAX,YMIN,YMAX,ZMIN,ZMAX,SUNKNO)
