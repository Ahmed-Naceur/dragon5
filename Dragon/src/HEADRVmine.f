...
*----
      ...
      LOGICAL LCHARG
      LOGICAL :: IsProvidedDENMESH = .FALSE.  
*----
*  ALLOCATABLE ARRAYS
*----
      REAL, ALLOCATABLE, DIMENSION(:) :: VOL,SGD,FLIN,DENMESH
...
*----
     ...
     1  //'ECORD LENGTH.')
      ...
      DO I=1,NPART
        ...
        CALL LCMLEN(IPMAC(I),'DENMESH',ILONG,ITYLCM)
        IF (ILONG.GT.0) THEN
           IsProvidedDENMESH = .TRUE.
           ALLOCATE(DENMESH(ILONG))
           CALL LCMGET(IPMAC(I),'DENMESH',DENMESH)
           DO II=1,SIZE(DENMESH)
              ! convert from g/cm3 to kg/cm3
              DENMESH(II)=DENMESH(II)*1.0E-03
           END DO
        ENDIF
     ...
     1      VOL(IBM)
            IF (IsProvidedDENMESH) THEN
            ! CASE OF CALL TO LIB: AND MICRO: PROVIDED FOR OUT:
               DOSEP(IBM,I)=DOSEP(IBM,I)+FLIN(IBM)*SGD(IBM)*ZNORM/
     1         VOL(IBM)*CONV/VOL(IBM)/DENMESH(IBM) 
            ELSE
               DOSEP(IBM,I)=DOSEP(IBM,I)+FLIN(IBM)*SGD(IBM)*ZNORM/
...
*     1  3*NPART+3)
         WRITE(IOUT,1001) '   REGION  ',(FUNA8(J),'_',SNAME(J),J=1,
     1  3*NPART+3)
        DO IBM=1,NBMIX
*          WRITE(IOUT,1002) VOL(IBM),(EDEPO(IBM,I),I=1,NPART),
          WRITE(IOUT,1002) IBM,(EDEPO(IBM,I),I=1,NPART),
