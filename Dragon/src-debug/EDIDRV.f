*DECK EDIDRV
     ...
     >                  NDEL,NALBP,ITRANC,NGCOND,NMERGE,IADF,NW,
...
*----
     ...
     >              IADF,NW,ICURR,IHF,IFFAC,ILUPS,NSAVES,NSTATS,
...
*----
     ...
     >              CTITLE*72,NAMSBR*12,HTYPE*8,TEXT12*12,HSMG*131
      ...
      INTEGER, ALLOCATABLE, DIMENSION(:) :: IHADF,IHADF2,ITYPE,MIX,
     > IDEPL,ISONA,ISONR,LSISO,INADPL,JPIFI,KDRI,INNAM,INNRF,NMIX
      INTEGER, ALLOCATABLE, DIMENSION(:,:,:) :: KEYANI
      REAL, ALLOCATABLE, DIMENSION(:) :: WORKF,WORKA,VOLME,WLETY,WE,
     > COURI,TAUXT,FLUXC,FADJC,SIGT,SIGS,SCATS,FLINT,SCATD,DEN,TN,EMEVF,
     > EMEVG,RER,DECAY,YIELD,PYIEL,RRD,FIYI,ENERG,NAWR,NDEN,NTMP,NVOL,
     > SNEJ,WORK1,WORK2
      REAL, ALLOCATABLE, DIMENSION(:,:,:) :: FLUXES,AFLUXE
...
*           use averaged fluxes obtained over boundary regions
            ...
            ALLOCATE(IHADF(NTYPE),IHADF2(2*NTYPE),COURI(NGCOND))
            CALL LCMGET(IPADF,'HADF',IHADF)
            DO IT=1,NTYPE
              WRITE(HTYPE,'(A4)') IHADF(IT)
              CALL EDIGAP(IPADF,HTYPE(:4),NGROUP,NGCOND,NREGIO,VOLUME,
     >        IGCOND,FLUXES,IPRINT,COURI)
              CALL LCMPUT(IPMAC2,HTYPE,NGCOND,2,COURI)
              READ(HTYPE,'(2A4)') IHADF2(2*IT-1),IHADF2(2*IT)
            ENDDO
            CALL LCMPUT(IPMAC2,'NTYPE',1,1,NTYPE)
            CALL LCMPUT(IPMAC2,'HADF',2*NTYPE,3,IHADF2)
            DEALLOCATE(COURI,IHADF2,IHADF)
...
*----
      NTAUXT=12+NW+2*NDEL
      ALLOCATE(TAUXT(NTAUXT*NELEMT),FLUXC((NW+1)*NELEMT),
     > FADJC((NW+1)*NELEMT),SIGS(NL*NELEMT),SCATS(NELEMT*NGCOND*NL),
