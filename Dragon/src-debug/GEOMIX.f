*DECK GEOMIX
      SUBROUTINE GEOMIX(LX,LY,LZ,LCOUR,MIX,MINMIX,MAXMIX)
*
*-----------------------------------------------------------------------
*
*Purpose:
  ...
  5   IZ2 = 0
      ...
      IF(INDIC.EQ.1) THEN
         ...
         IF( IZ1.LT.1.OR.IZ1.GT.LZ )THEN
            CALL XABORT('GEOMIX: INVALID PLANE NUMBER'//
     >                  '(GREATER THAN *LZ*).')
         ENDIF
      ELSE
         CALL XABORT('GEOMIX: PLANE NUMBER MUST BE READ'//
     >               '(INTEGER EXPECTED).')
      ENDIF
      CALL REDGET(INDIC,NITMA,FLOTT,TEXT12,DFLOTT)
      IF( INDIC.EQ.3 )THEN
     ...
     >                        '(GREATER THAN PREVIOUS).')
               ...
               GOTO 20
            ELSE
               CALL XABORT('GEOMIX: SAME AS WHICH PLANE? '//
     >               '(INTEGER EXPECTED).')
            ENDIF
         ELSEIF((TEXT12.EQ.'CROWN'.OR.TEXT12.EQ.'UPTO').AND.LCOUR) THEN
            ...
            IF( TEXT12.EQ.'UPTO' )THEN
               CALL REDGET(INDIC,NITMA,FLOTT,TEXT12,DFLOTT)
               IF(INDIC.NE.1) CALL XABORT('GEOMIX: INTEGER DATA'//
     ...
     >         THEN
            CALL XABORT('GEOMIX: UNSUPPORTED KEYWORD *CROWN* OR *UPTO*'
     >                 //': HEX3D COMPLETE ONLY')
         ELSE
            CALL XABORT('GEOMIX: INVALID CHARACTER VARIABLE '//TEXT12)
         ENDIF
      ELSEIF (INDIC.EQ.1) THEN
         GOTO 20
      ELSE
         CALL XABORT('GEOMIX: INTEGER OR CHARACTER VARIABLE EXPECTED')
      ENDIF
*----
*  READ A CWOWN
*----
 ...
 42            CONTINUE
               NC= NC -1
            ELSE
               CALL XABORT('GEOMIX: ALL OF WHICH MIX? '//
     >               '(INTEGER EXPECTED).')
            ENDIF
          ELSE
            CALL XABORT('GEOMIX: KEYWORD *SAME* OR *ALL* '//
     >                  '(CHARACTER EXPECTED).')
          ENDIF
      ELSEIF( INDIC.EQ.1 )THEN
        IF( NCSAME.NE.1 )THEN
           CALL XABORT('GEOMIX: INVALID INTEGER WITH *UPTO* ')
        ENDIF
        IF( NC.EQ.0 )THEN
           MIX(IHEX+1,1,IZ1)= NITMA
           IHEX= IHEX+1
        ELSE
         ...
         IF(.NOT.(IX.EQ.1) ) THEN
          ...
          IF(INDIC.NE.1)THEN
           ...
           CALL XABORT('GEOMIX: INTEGER DATA EXPECTED(1)')
          ENDIF
         ENDIF
 ...
 33      CONTINUE
        ...
        ENDIF
      ELSE
        CALL XABORT('GEOMIX: MIXTURE # EXPECTED '//
     >              '(INTEGER EXPECTED).')
      ENDIF
      IF( IHEX.LT.LX )THEN
         CALL REDGET(INDIC,NITMA,FLOTT,TEXT12,DFLOTT)
         IF(INDIC.NE.3)THEN
            ...
            CALL XABORT('GEOMIX: KEYWORD *CROWN* OR *UPTO*'//
     >                  ' MUST BE READ.')
         ENDIF
     ...
     >                  ' MUST BE READ.')
      ...
      ELSEIF( IHEX.EQ.LX )THEN
         GO TO 25
      ELSE
         CALL XABORT('GEOMIX: INVALID # OF MIX IN THIS PLANE.')
      ENDIF
*----
       ...
       DO 22 IY=1,LY
        DO 21 IX=1,LX
         IF(.NOT.((IX.EQ.1).AND.(IY.EQ.1))) THEN
           CALL REDGET(INDIC,NITMA,FLOTT,TEXT12,DFLOTT)
           IF(INDIC.NE.1) THEN
             CALL XABORT('GEOMIX: INTEGER DATA EXPECTED(2)')
           ENDIF
         ENDIF
         MIX(IX,IY,IZ1) = NITMA
         MAXMIX=MAX(MAXMIX,NITMA)
 ...
 25   CONTINUE
      IF (NZ.LT.LZ) THEN
         ...
         IF(INDIC.NE.3.OR.TEXT12.NE.'PLANE') THEN
            CALL XABORT('GEOMIX: KEYWORD *PLANE* MUST BE READ.')
         ENDIF
         NC= -1
         IHEX= 0
         GO TO 5
      ENDIF
      IF (NZ.NE.LZ) CALL XABORT('GEOMIX: WRONG NUMBER OF PLANES')
*
      RETURN
      END
