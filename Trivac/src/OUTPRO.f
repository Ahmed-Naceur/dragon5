...
*----
   ...
   40 CONTINUE
      DO 60 IGR=1,NGRP
      DO 50 IBM=1,NZS
      AFLINT(IBM,IGR)=AFLINT(IBM,IGR)/VOLI(IBM)
   50 CONTINUE
   60 CONTINUE
*----
...
*----
      ...
      IGRDEB=IGRFIN+1
      IGRFIN=IGCOND(IGRC)
      CALL XDRSET(OUTR,(NZS+1)*(NREAC+5),0.0)
      CALL XDRSET(OUTSC,NZS*(NL+2)*NGCOND,0.0)
...
*----
     ...
     1   //'FOR FISSION SPECTRUM.')
         ...
         IF(LENGT.EQ.0) THEN
            IF(IGR.EQ.IGRDEB) THEN
               CALL XDRSET(OUTR(1,NREAC+4),NZS,1.0)
            ENDIF
         ELSE
            ALLOCATE(DEN(NZS))
  ...
  270       CONTINUE
            DO 280 IBM=1,NZS
            IF(DEN(IBM).NE.0.0) OUTR(IBM,NREAC+4)=RATE(IBM)/DEN(IBM)
  280       CONTINUE
            DEALLOCATE(DEN)
         ENDIF
