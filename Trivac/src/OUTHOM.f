...
*----
      ...
      INTEGER, DIMENSION(:), ALLOCATABLE :: ISPLX,ISPLY,ISPLZ,MAT2,DP,MX
      INTEGER, DIMENSION(:), ALLOCATABLE :: XXX,YYY,ZZZ
...
*----
     ...
     1   //'T ERROR.')
         ALLOCATE(DP(MAXNEL),MX(NELOLD))
         DO 205 I=1,NELOLD
         MX(I)=IHOM(I)
  205    CONTINUE
         LXOLD=LX1
         CALL BIVALL(MAXNEL,IHEX,LXOLD,LX,DP)
         DO 215 KZ=1,LZOLD
         DO 210 KX=1,LX
         IHOM(KX+(KZ-1)*LX)=0
         KEL=DP(KX)+(KZ-1)*LXOLD
